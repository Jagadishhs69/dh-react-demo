...
class LocalPipelineRunner {
    ...
    async runNodeCodeQuality() {
        ...
        results.eslint = await this.runCommand('npx eslint . --ext .js,.jsx,.ts,.tsx --no-error-on-unmatched-pattern', { 
            cwd: workingDir,
            ignoreStderr: true 
        });
        ...
        results.prettier = await this.runCommand('npx prettier --check . --ignore-unknown', { 
            cwd: workingDir,
            ignoreStderr: true 
        });
    ...
    async runBasicSyntaxCheck(workingDir) {
        ...
        try {
            findJsFiles(workingDir);
            
            if (jsFiles.length === 0) {
                return { success: true, message: 'No JavaScript files found to check', fileCount: 0 };
            }
            ...
            for (const file of jsFiles) {
                try {
                    ...
                    syntaxErrors++;
                    errors.push(`${path.relative(workingDir, file)}: ${error.message}`);
                }
            }
        ...
        if (languages.includes('nodejs')) {
            ...
            if (!results.nodejs.success) {
                ...
                if (results.nodejs.vulnerablePackages && results.nodejs.vulnerablePackages.length > 0) {
                    results.nodejs.vulnerablePackages.forEach(pkg => {
                        this.logToStage('npm-version-scan', 'error', `Vulnerable package: ${pkg}`);
                    });
                }
            }
        } else {
        ...
        if (auditResult.stdout) {
            try {
                ...
                if (auditData.vulnerabilities) {
                    vulnerablePackages = Object.keys(auditData.vulnerabilities);
                }
            } catch (error) {
                this.log('debug', 'Could not parse npm audit output');
            }
        }
