...
class CodeReviewGenerator {
                    ...
                    Object.entries(result.results).forEach(([tool, toolResult]) => {
                        if (!toolResult.success && !toolResult.skipped) {
                            if (tool === 'eslint') {
                                ...
                                // Extract detailed error information from ESLint
                                if (toolResult.detailedErrors && Array.isArray(toolResult.detailedErrors)) {
                                    analysis.detailedErrors.push(...toolResult.detailedErrors);
                                } else if (toolResult.error && typeof toolResult.error === 'string') {
                                    // Fallback: parse error string if detailedErrors not available
                                    const errorLines = toolResult.error.split('\n');
    ...
    generateDetailedIssues(reportData) {
        let detailsMarkdown = '';
        
        // Collect all critical and high-priority issues with details
        const criticalIssues = [];
        const highPriorityIssues = [];
        const allIssues = [];
        
        Object.entries(reportData.stages).forEach(([stageName, stage]) => {
            if (stage.issues && stage.issues.length > 0) {
                stage.issues.forEach(issue => {
                    const issueDetail = {
                        ...
                        category: this.determineCategory(stageName)
                    };
                    
                    if (issueDetail.severity === 'critical') {
                        criticalIssues.push(issueDetail);
                    } else if (issueDetail.severity === 'high') {
                        highPriorityIssues.push(issueDetail);
                    }
                    allIssues.push(issueDetail);
                });
            }
        });
        
        if (criticalIssues.length === 0 && highPriorityIssues.length === 0 && allIssues.length === 0) {
            return '✅ **No critical issues found!** All quality checks passed successfully.';
        }
        
        // Generate critical issues section
        if (criticalIssues.length > 0) {
            detailsMarkdown += '### 🚨 Critical Issues (Must Fix)\n\n';
            criticalIssues.forEach((issue, index) => {
                detailsMarkdown += `**${index + 1}. ${issue.category} - ${issue.stage}**\n`;
                detailsMarkdown += `- **Issue:** ${issue.issue}\n`;
                detailsMarkdown += `- **Severity:** 🚨 Critical\n`;
                detailsMarkdown += `- **Action Required:** Immediate fix needed\n\n`;
            });
        }
        
        // Generate high priority issues section
        if (highPriorityIssues.length > 0) {
            detailsMarkdown += '### ⚠️ High Priority Issues\n\n';
            highPriorityIssues.forEach((issue, index) => {
                detailsMarkdown += `**${index + 1}. ${issue.category} - ${issue.stage}**\n`;
                detailsMarkdown += `- **Issue:** ${issue.issue}\n`;
                detailsMarkdown += `- **Severity:** ⚠️ High\n`;
                detailsMarkdown += `- **Recommendation:** Fix before next release\n\n`;
            });
        }
        
        // Add detailed error information if available
        const detailedErrors = this.extractDetailedErrors(reportData);
        if (detailedErrors.length > 0) {
            detailsMarkdown += '### 📍 Specific Error Locations\n\n';
            detailedErrors.forEach((error, index) => {
                detailsMarkdown += `**${index + 1}. ${error.file}:${error.line}:${error.column}**\n`;
                detailsMarkdown += `- **Tool:** ${error.tool}\n`;
                detailsMarkdown += `- **Error:** ${error.message}\n`;
                detailsMarkdown += `- **Location:** Line ${error.line}, Column ${error.column}\n\n`;
            });
        }
        
        // File-specific recommendations section removed as requested
        
        return detailsMarkdown;
    }
    
    determineSeverity(issue) {
        const criticalKeywords = ['security', 'vulnerability', 'syntax error', 'cannot resolve', 'module not found'];
        const highKeywords = ['eslint', 'configuration missing', 'deprecated', 'outdated'];
    ...
    determineCategory(stageName) {
        const categoryMap = {
            'languageDetection': 'Project Setup',
            'dependencyCheck': 'Dependencies',
            'codeQuality': 'Code Quality',
            'npmVersionScan': 'Package Management',
            'secretDetection': 'Security',
            'sastScan': 'Security Analysis'
        };
        return categoryMap[stageName] || 'General';
    }
    
    extractDetailedErrors(reportData) {
        const detailedErrors = [];
        
        // Extract detailed errors from code quality stage
        if (reportData.stages.codeQuality && reportData.stages.codeQuality.detailedErrors) {
            detailedErrors.push(...reportData.stages.codeQuality.detailedErrors);
        }
        
        // Extract detailed errors from other stages that might have them
        Object.values(reportData.stages).forEach(stage => {
            if (stage.detailedErrors && Array.isArray(stage.detailedErrors)) {
                detailedErrors.push(...stage.detailedErrors);
            }
        });
...
${this.generateDetailedIssues(reportData)}
